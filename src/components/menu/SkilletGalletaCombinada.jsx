...
const GALLETAS_MEDIANA = ["Caramelo", "Chocolate", "MYM", "Trozos", "Macadamia"];
...
export default function SkilletGalletaCombinada({ product, open, onClose, onAdd }) {
  const [galletaMed, setGalletaMed] = useState(null);
  const [galletaSup, setGalletaSup] = useState(null);
  const [helado, setHelado] = useState(null);
  const [salsa, setSalsa] = useState(null);
  const [openSec, setOpenSec] = useState("galletaMed");

  if (!product) return null;

  const ready = galletaMed && galletaSup && helado && salsa;

  const handleConfirm = () => {
    if (!ready) return;
    const notes = `Galleta Mediana: ${galletaMed} | Galleta Suprema: ${galletaSup} | Helado: ${helado} | Salsa: ${salsa} (Gratis)`;
    ...
    <Sheet open={open} onOpenChange={onClose}>
      <SheetContent side="bottom" className="rounded-t-3xl" style={{ background: "#FFFCFD", border: "none", maxHeight: "85vh", overflowY: "auto", padding: 0 }}>
        ...
        <div style={{ borderBottom: "1px solid #F0E4EA" }}>
          <button 
            onClick={() => setOpenSec(s => s === "galletaMed" ? null : "galletaMed")}
            style={{ width: "100%", display: "flex", justifyContent: "space-between", padding: "14px 16px", background: "none", border: "none", cursor: "pointer" }}
          >
            <span style={{ fontSize: 14, fontWeight: 700, color: "#1A0A10" }}>Galleta Mediana {galletaMed && `✓ ${galletaMed}`}</span>
            <span style={{ fontSize: 18, color: "#BBA8B0" }}>{openSec === "galletaMed" ? "∧" : "∨"}</span>
          </button>
          {openSec === "galletaMed" && GALLETAS_MEDIANA.map(g => (
            <RadioOption key={g} label={g} selected={galletaMed === g} onSelect={() => { setGalletaMed(g); setOpenSec("galletaSup"); }} />
          ))}
        </div>
        ...
        <div style={{ borderBottom: "1px solid #F0E4EA" }}>
          <button 
            onClick={() => setOpenSec(s => s === "galletaSup" ? null : "galletaSup")}
            style={{ width: "100%", display: "flex", justifyContent: "space-between", padding: "14px 16px", background: "none", border: "none", cursor: "pointer" }}
          >
            <span style={{ fontSize: 14, fontWeight: 700, color: "#1A0A10" }}>Galleta Suprema {galletaSup && `✓ ${galletaSup}`}</span>
            <span style={{ fontSize: 18, color: "#BBA8B0" }}>{openSec === "galletaSup" ? "∧" : "∨"}</span>
          </button>
          {openSec === "galletaSup" && GALLETAS_SUPREMA.map(g => (
            <RadioOption key={g} label={g} selected={galletaSup === g} onSelect={() => { setGalletaSup(g); setOpenSec("helado"); }} />
          ))}
        </div>
        ...
        <div style={{ padding: "16px", position: "sticky", bottom: 0, background: "#FFFCFD", borderTop: "1px solid #F0E4EA" }}>
          {!ready && <p style={{ fontSize: 11, color: "#BBA8B0", textAlign: "center", marginBottom: 8 }}>* Elige todas las opciones</p>}
          <button
