...
const GALLETAS_MEDIANAS = ["Vainilla Mediana", "Chocolate Mediana", "Fresa Mediana", "Brownie Mediana"];
...
export default function GalletaMediumHelladoCustomizer({ product, open, onClose, onAdd }) {
  ...
  const ready = galleta && helado;

  const handleConfirm = () => {
    if (!ready) return;
    const notes = `Galleta: ${galleta} | Helado: ${helado}`;
    ...
    <Sheet open={open} onOpenChange={onClose}>
      <SheetContent side="bottom" className="rounded-t-3xl" style={{ background: "#FFFCFD", border: "none", maxHeight: "85vh", overflowY: "auto", padding: 0 }}>
        ...
        <div style={{ borderBottom: "1px solid #F0E4EA" }}>
          ...
          >
            <span style={{ fontSize: 14, fontWeight: 700, color: "#1A0A10" }}>Galleta {galleta && `✓ ${galleta}`}</span>
            <span style={{ fontSize: 18, color: "#BBA8B0" }}>{openSec === "galleta" ? "∧" : "∨"}</span>
          </button>
          {openSec === "galleta" && GALLETAS_MEDIANAS.map(g => (
            <RadioOption key={g} label={g} price={0} selected={galleta === g} onSelect={() => { setGalleta(g); setOpenSec("helado"); }} />
          ))}
        </div>
        ...
        <div style={{ borderBottom: "1px solid #F0E4EA" }}>
          ...
          {openSec === "helado" && SABORES_HELADO.map(h => (
            <RadioOption key={h} label={h} price={0} selected={helado === h} onSelect={() => setHelado(h)} />
          ))}
        </div>

        {/* Footer */}
        <div style={{ padding: "16px", position: "sticky", bottom: 0, background: "#FFFCFD", borderTop: "1px solid #F0E4EA" }}>
          {!ready && <p style={{ fontSize: 11, color: "#BBA8B0", textAlign: "center", marginBottom: 8 }}>* Elige galleta y helado</p>}
          <button
