...
export default function SkilletGalletaMedianaCustomizer({ product, open, onClose, onAdd }) {
  ...
  const handleConfirm = () => {
    if (!ready) return;
    const notes = `Galleta Mediana: ${galleta} | Helado: ${helado} | Salsa: ${salsa} (Gratis)`;
    ...
    <Sheet open={open} onOpenChange={onClose}>
      <SheetContent side="bottom" className="rounded-t-3xl" style={{ background: "#FFFCFD", border: "none", maxHeight: "85vh", overflowY: "auto", padding: 0 }}>
        ...
        <div style={{ borderBottom: "1px solid #F0E4EA" }}>
          ...
          >
            <span style={{ fontSize: 14, fontWeight: 700, color: "#1A0A10" }}>Galleta Mediana {galleta && `✓ ${galleta}`}</span>
            <span style={{ fontSize: 18, color: "#BBA8B0" }}>{openSec === "galleta" ? "∧" : "∨"}</span>
          </button>
          {openSec === "galleta" && GALLETAS_MEDIANA.map(g => (
